...
function change_product_weight_price(price ,weight,unit){
	jQuery('#weight_id').val(weight);
	jQuery('#price_id').val(price);
	jQuery('#unit_id').val(unit);
	jQuery('.weight-price').html('<a href="javascript:void(0)" class="btn btn-success" onclick=clear_weight_price()>Clear</a></br> <span> <b> Price: </b></span> &nbsp;&nbsp;'+price);
	jQuery('.weight_link').css('color','#333');
	jQuery('#weight_'+weight).css('color','#41d03687');

}

function clear_weight_price(){
	jQuery('.weight-price').html('<div class="weight-price"></div>');
	jQuery('.weight_link').css('color','#333');
}

function sort_by(){
	var sort_by_value =jQuery('#sort_by_val').val();
...
function select_amoute(amount){
	jQuery('#sp_amount').val(amount);
}


function applyCouponCode(){
	var coupon_code =jQuery('#coupon_code').val();
	jQuery('#coupon_code_str').val(coupon_code);
}
